...
import {
  buildSnapshotCalendar,
  hasCalendarData,
} from '../utils/leetcodeProfile';

const Heatmap = ({ calendar, stats, filter, onFilterChange, streakInfo }) => {
  const hasLiveCalendar = hasCalendarData(calendar);
  const visualCalendar = hasLiveCalendar
    ? calendar
    : buildSnapshotCalendar(stats);

  const processedData = useMemo(() => {
    if (!visualCalendar) return null;
    ...
    const submissions = visualCalendar || {};
    for (let i = 0; i < 365; i++) {
  ...
  }, [visualCalendar, filter]);

  const totalSubmissions = processedData
    ? processedData.heatmap.reduce((sum, day) => sum + day.count, 0)
    : streakInfo?.totalSubmissions || stats?.totalSubmissionsPastYear || 0;
  ...
  const getLevelColor = (level) => {
    switch (level) {
      ...
      case 2:
        return 'bg-[#40c463]';
      case 1:
        return 'bg-[#9be9a8]'; // Lightest Green
      default:
        return 'bg-white/5'; // Empty
    }
  };
      ...
      >
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
            ...
            <div className="flex items-center gap-6 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
              <span>
                ...
                <span className="text-white">
                  {streakInfo?.totalActive || stats?.totalActiveDays || 0}
                </span>
              </span>
              <span>
                Max streak:{' '}
                <span className="text-white">
                  {streakInfo?.max || stats?.maxStreak || 0}
                </span>
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              className="bg-white/5 border border-white/10 rounded-2xl px-4 py-2 text-sm text-white outline-none"
            >
        ...
        <div className="rounded-4xl border border-white/10 bg-white/5 p-6 text-gray-300">
          ...
          <p className="text-sm text-gray-400 leading-7">
            The public LeetCode page exposes the profile summary, but the
            browser can’t reliably read the full day-by-day contribution heatmap
            from the blocked API source.
          </p>
        </div>
      </motion.div>
    );
  ...
  return (
    ...
    >
      ...
      <div className="overflow-x-auto pb-4 scrollbar-hide">
        <div className="min-w-187.5 pr-8">
          <div className="grid grid-rows-7 grid-flow-col gap-1.25 mb-6">
            {processedData.heatmap.map((day, i) => (
              <motion.div
                ...
                className={`w-3.5 h-3.5 rounded-sm transition-all duration-300 ${getLevelColor(day.level)} cursor-crosshair group relative`}
              >
          ...
          <div className="relative h-6 flex items-center text-[10px] font-black text-gray-500 uppercase tracking-[0.3em]">
            {processedData.monthLabels.map((lbl, idx) => (
              <span
                key={idx}
                className="absolute inline-block w-8 text-center -translate-x-1/2"
                style={{ left: `${(lbl.col / 51) * 100}%` }}
              >
                {lbl.name}
              </span>
            ))}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-8 pt-6 border-t border-white/10 text-gray-400">
        ...
        <div className="flex items-center gap-3">
          ...
          {[0, 1, 2, 3, 4].map((l) => (
            <div
              key={l}
              className={`w-3.5 h-3.5 rounded-sm ${getLevelColor(l)} transition-transform hover:scale-125 border border-black/5`}
            ></div>
          ))}
        </div>
      </div>
