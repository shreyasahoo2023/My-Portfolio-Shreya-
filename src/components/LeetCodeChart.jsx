...
const LeetCodeChart = ({ stats }) => {
  ...
  const hasSubmissionCalendar = hasCalendarData(stats.submissionCalendar);
  const barData = hasSubmissionCalendar
    ? buildMonthlyActivity(stats.submissionCalendar)
    : buildSnapshotMonthlyActivity(stats);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
      ...
      >
        <div className="flex items-center justify-between mb-8">
          <h3 className="text-white text-xs font-black uppercase tracking-widest flex items-center gap-2">
            <PieIcon size={16} className="text-accent-pink" /> Problem
            Distribution
          </h3>
      ...
      >
        <div className="flex items-center justify-between mb-8">
          <h3 className="text-white text-xs font-black uppercase tracking-widest flex items-center gap-2">
            <TrendingUp size={16} className="text-accent-blue" /> Monthly
            Submission Activity
          </h3>
          <div className="px-3 py-1 bg-white/5 rounded-full border border-white/10">
            <span className="text-[10px] text-[#ffa116] font-black uppercase tracking-widest">
              {stats.totalSubmissionsPastYear || stats.totalSolved} in 12 months
            </span>
          </div>
        </div>
        ...
        <div className="mt-8 flex items-center justify-between text-[10px] font-black text-gray-500 uppercase tracking-widest">
          <span>
            {hasSubmissionCalendar ? 'Growth Curve 2025' : 'Profile Snapshot'}
          </span>
